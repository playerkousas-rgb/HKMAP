...
- 放置起點 △、檢查點 ○、終點 ◎，拖移調整
- 自動計算 **平面距離**、方格方位、磁方位（約 3.1°W）
- 即時坐標：WGS84、**UTM 方格（KK／QK 8 位，同 HM20C）**、6 位／8 位方格
- 可疊加 **UTM 1 km 方格網**（zone 49Q／50Q，GE／HE／JK／KK）
- 標準比例 **1:5 000–1:20 000**：選定即對齊，可 **LOCK 比例**（🔒 停用縮放，比例不會飄成 1:18 990 之類）；白紙上就是該比例印出
- **設計＝列印（WYSIWYG）**：畫面中央的白紙就是實際列印頁（A3/A4/A5、可切換直向／橫向、實際 mm 尺寸），紙上見到什麼就印什麼，紙外不會印；說明表分開第 2 張紙
- CP 走出白紙會即時顯示 ⚠ 警告（一鍵「對齊路線」），列印前再次把關——設計不可能超出列印範圍
- 路線 JSON／GPX 匯出、匯入、列印檢查點說明表（第 2 頁）
- 本機暫存（瀏覽器 localStorage）
- **返回／重做**（Ctrl+Z / Ctrl+Y）、清除檢查點、清除本機暫存（解除瀏覽器暫存卡住）
...
4. **列印地圖**（`index.html` 內直接列印，設計＝列印）：紙圖上套印紫紅 CP 圓圈與檢查點說明，給參加者使用
